...
class GoogleSheetsPaymentRepository:
    ...
    def create_pending_order(
        ...
        *,
        ...
        existing = self.find_by_external_reference(external_reference)
        ...
        self._append(
            self._worksheet(PAYMENT_ORDERS_SHEET),
            {
                **order.__dict__,
    ...
    def find_by_external_reference(self, external_reference: str) -> StoredPaymentOrder | None:
        for row in self._records(PAYMENT_ORDERS_SHEET):
    ...
    def find_by_provider_order_id(self, provider_order_id: str) -> StoredPaymentOrder | None:
        for row in self._records(PAYMENT_ORDERS_SHEET):
            if str(row.get("provider_order_id", "")) == provider_order_id:
                return self._to_order(row)
        return None
    ...
    def append_payment_event(
        ...
        *,
        ...
        self._append(
            self._worksheet(PAYMENT_EVENTS_SHEET),
            {
    ...
    def record_webhook(
        ...
        *,
        ...
        if provider_event_id and any(
            str(row.get("provider_event_id", "")) == provider_event_id
            for row in self._records(WEBHOOK_EVENTS_SHEET)
        ):
            return False
        self._append(
            self._worksheet(WEBHOOK_EVENTS_SHEET),
            {
    ...
    def _find_row(self, key: str, value: str) -> tuple[int, dict[str, Any]]:
        for row_number, row in enumerate(
            self._worksheet(PAYMENT_ORDERS_SHEET).get_all_records(default_blank=""),
            start=2,
        ):
