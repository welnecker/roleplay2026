...
def test_bridge_saves_user_and_mary_as_one_capsule() -> None:
    ...
    assert episode["eligible_after_turn"] == 4


def test_capsule_stays_dormant_while_exchange_is_in_recent_history() -> None:
    facts: dict[str, str] = {}
    _turn(facts)
    prepare_bridge_episode(
...
def test_terms_are_not_predeclared_or_topic_specific() -> None:
    ...
    assert "lua" not in episode["anchors"]  # termos curtos não são necessários para o fio
