...
class FletApiClient:
    ...
    def _run_frame(self, response: requests.Response) -> ApiRunFrame:
        ...
        return ApiRunFrame(
            ...
            motion_url=self._public_media_url(payload.get("motion_url", "")),
            audio_url=self._public_media_url(payload.get("audio_url", "")),
            ...
            entry_audio_urls=tuple(
                self._public_media_url(item)
                for item in payload.get("entry_audio_urls", []) or []
            ),
            entry_effect_audio_urls=tuple(
                tuple(self._public_media_url(item) for item in urls or [])
                for urls in payload.get("entry_effect_audio_urls", []) or []
            ),
