...
class RunFrameResponse(BaseModel):
    ...
    entry_effect_audio_urls: list[list[str]] = Field(default_factory=list)
    revealed_entries: int
    entry_count: int
    finished: bool
...
def _run_response(frame: RunFrame, request: Request) -> RunFrameResponse:
    ...
    return RunFrameResponse(
        ...
        entry_effect_audio_urls=[
            [absolute(url) for url in urls] for urls in frame.entry_effect_audio_urls
        ],
