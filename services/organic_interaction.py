...
_EXPLICIT_NAME_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("me_chamo", re.compile(rf"\bme\s+chamo\s+(?P<name>{_NAME_TOKEN})\b", re.IGNORECASE)),
    ("meu_nome_e", re.compile(rf"\bmeu\s+nome\s+[ée]\s+(?P<name>{_NAME_TOKEN})\b", re.IGNORECASE)),
    ("pode_me_chamar", re.compile(rf"\bpode\s+me\s+chamar\s+de\s+(?P<name>{_NAME_TOKEN})\b", re.IGNORECASE)),
)
...
_THOUGHT_BLOCK_PATTERN = re.compile(r"\[PENSAMENTO\].*?\[/PENSAMENTO\]", re.IGNORECASE | re.DOTALL)
...
def extract_user_facts(user_text: str, known_facts: dict[str, str] | None = None) -> dict[str, str]:
...
def extract_assistant_facts(assistant_text: str, known_facts: dict[str, str] | None = None) -> dict[str, str]:
    facts = dict(known_facts or {})
...
def detect_organic_signal(user_text: str, known_facts: dict[str, str] | None = None) -> OrganicSignal | None:
    ...
    if evidence is not None and evidence.value.casefold() != acknowledged_name.casefold():
        ...
        return OrganicSignal(
            ...
            facts=facts,
            instruction=(f"Reconheça claramente que o nome do usuário é {evidence.value}. Use o nome na resposta. "
                         "Só diga que você se chama Mary se esse fato ainda não estiver registrado. "
    ...
    if known_name and any(term in lowered for term in ("soletra", "soletrar", "como se escreve", "letra por letra")):
        ...
        return OrganicSignal(
            kind="related_challenge", facts=facts,
            instruction=f"Aceite o desafio com humor e soletre o nome corretamente: {spelling}. Não execute a próxima linha canônica nesta mesma resposta.",
    ...
    if len(text.split()) >= 3 and any(pattern.search(text) for pattern in _EXCLUSIVE_REACTION_PATTERNS):
        return OrganicSignal(
            kind="free_reaction", facts=facts,
            instruction=("Reaja exclusivamente à preocupação, ressalva ou comentário sensível do usuário. "
    ...
    if len(text.split()) >= 3 and any(pattern.search(text) for pattern in _INTEGRATED_REACTION_PATTERNS):
        return OrganicSignal(
            kind="integrated_reaction", facts=facts,
            instruction=("Responda primeiro ao conteúdo específico do usuário em uma ou duas frases curtas e naturais. "
    ...
    if "?" in text and len(text.split()) >= 3:
        return OrganicSignal(
            kind="direct_question", facts=facts,
            instruction=("Responda primeiro à pergunta direta do usuário de forma curta e natural. "
...
def render_facts(facts: dict[str, Any]) -> str:
    items = [f"{key}={value}" for key, value in facts.items() if not str(key).startswith("_") and str(value).strip()]
