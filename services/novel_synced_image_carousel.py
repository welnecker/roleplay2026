...
def _clean_image_id(value: object) -> str:
    return str(value or "").strip().replace("\\", "/")


def _row_image_maps(rows: Iterable[dict[str, Any]]) -> tuple[dict[str, str], dict[str, str]]:
    ...
    for raw in rows:
        ...
        if kind == "descricao":
            description_index += 1
        image_id = _clean_image_id(row.get("image_id"))
...
def enrich_compiled_document_with_image_ids(
    ...
    """Copia ``image_id`` opcional para o payload V2 sem mudar roteiros legados."""
        ...
        for beat in block.get("beats", []) or []:
            ...
            for entry in frame.get("entries", []) or []:
                if not isinstance(entry, dict):
                    continue
                entry_image = line_images.get(str(entry.get("line_id", "") or "").strip(), "")
                if entry_image:
...
def image_sequence_for_frame(
    ...
    *,
    ...
    """Retorna imagem-base e imagem efetiva de cada balão com carry-forward."""

    last = _clean_image_id(frame.get("image_id")) or _clean_image_id(inherited_image_id)
    base = last
    result: list[str] = []
    for raw in frame.get("entries", []) or []:
        entry = raw if isinstance(raw, Mapping) else {}
        own = _clean_image_id(entry.get("image_id"))
        if own:
            last = own
        result.append(last)
...
def _resolve_image_id(package_id: str, image_id: str) -> dict[str, object] | None:
    ...
    if relative.suffix:
        ...
        for ext in ("webp", "png", "jpg", "jpeg"):
            candidates.extend((scene_root / f"{clean}.{ext}", root / f"{clean}.{ext}"))
